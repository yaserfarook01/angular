...
describe('AddLaptopComponent', () => { // Update component name
  ...
  fit('should_add_a_new_laptop_when_form_is_valid', () => { // Update test description
    ...
    spyOn(service, 'addLaptop').and.returnValue(of(mockLaptop)); 
    
    (component as any).laptopForm.setValue(mockLaptop); // Set form values
    (component as any).addNewLaptop(); // Trigger the addNewLaptop method
    
    expect((component as any).laptopForm.valid).toBeTruthy();
    expect((service as any).addLaptop).toHaveBeenCalledWith(mockLaptop);
  });

  fit('should_require_all_form_fields_to_be_filled_in', () => { // Update test description
    const form = (component as any).laptopForm; // Update form name
