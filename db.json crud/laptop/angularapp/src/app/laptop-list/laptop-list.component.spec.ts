...
describe('LaptopListComponent', () => {
  ...
  fit('should_call_getLaptops', () => {
    ...
    expect(service.getLaptops).toHaveBeenCalled();
  });

  fit('should_call_deleteLaptop', () => {
    spyOn(service, 'deleteLaptop').and.returnValue(of());
    (component as any).deleteLaptop(1); // Passing a valid laptop ID
    expect((component as any).deleteLaptop).toBeDefined();
    expect((component as any).deleteLaptop instanceof Function).toBeTruthy();
    expect(service.deleteLaptop).toHaveBeenCalledWith(1);
  });
